...
const ChordLessonPage = ({ onNavigate }) => {
    ...
    const processChordResponse = useCallback(async (response) => {
        try {
            ...
            // 處理錯誤情況 - 後端要求切換回整個和弦模式
            if (response.success === false) {
                console.log('後端返回錯誤，強制切換回整個和弦模式:', response.error);
                // 強制設置為整個和弦模式
                currentChordRef.current = response.target_chord || currentChordRef.current;
                wholeChordRef.current = true; // 強制設置為整個和弦模式
                currentStringRef.current = null; // 清除弦號

                setState(prev => ({
                    ...prev,
                    currentChord: response.target_chord || prev.currentChord,
                    wholeChord: true,
                    currentString: null,
                    error: null, // 清除錯誤狀態，讓用戶重新嘗試
                    phase: 'idle'
                }));
                return;
            }
